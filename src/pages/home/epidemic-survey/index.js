...
import {
  ...
  callXG22ListApi, callXGPersonSort, callXGPersonSortApi,
  getJobCaseDicApi,
  getPatientTypeApi
} from "../../../services/home";
...
const EpidemicSurvey = () => {
  ...
  useEffect(() => {
    ...
    Taro.eventCenter.on('appoint-info', item => {
      const {
        idCard,
        name,
        phone,
    ...
    initData();
    return () => {
      Taro.eventCenter.off('appoint-info');
    }
  }, [])
  ...
  const nextStep = async () => {
    ...
    if ($res.code === 200) {
      ...
      if (res.code == 200) {
        Taro.navigateTo({
          url: `/pages/home/epidemic-success/index?data=${res.data}`
        })
  ...
  const clinicDeptChange = (e) => {
    setClinicDept(clinicDeptRange[e.detail.value].deptName);
    setClinicDeptId(clinicDeptRange[e.detail.value].deptCode);
  }
  ...
  return (
    <View className='container'>
      <ScrollView showScrollbar={false} enhanced={true}>
          ...
          <View className='address-info-view' style='justify-content:flex-start'>
            ...
            <RadioGroup onChange={selectIdentity}>
              <View style='display:flex;flexWrap:wrap;justifyContent:flex-start'>
                ...
                <Radio style={{transform: 'scale(0.8)', color: '#333',}}
                       color='#06B48D' value='陪护' checked={identity == '陪护'}>陪护</Radio>
              </View>
            </RadioGroup>
          </View></View>
        <View className='line'/>
        ...
        <View className='address-info-container' onClick={() => {
          Taro.navigateTo({
            url: `/pages/home/invest-choice-patient/index`
          })
        }}>
          <View className='address-info-wrap'>
            <View className='address-info-view'>
              <View style='display:flex;alignItems:center'>
                <Text style='font-size:14PX;color:red;margin-right:2PX;'>*</Text>
                <Text className='dist-name-text'>您的姓名</Text>
                <Text className='select-city-text list-row-input'
                      style={name === '' ? 'color:#999' : 'color:#666'}>{isEmpty(name) ? '请选择预约人姓名' : name}</Text>
              </View>
...
const ListRow = (props) => {
  const {label, placeholder, disabled, value, required, className, type, onInput} = props;
  return (
    <View className='list-row-container'>
      <View className='list-row-wrap'>
