...
export default class choicePatient extends PureComponent {
  ...
  onAppointConfirm=async (row)=>{
    let {item}=getCurrentInstance().router.params;
    const {sourceId, orgId, date, appointId,comboName, orgName, timeType, price} = JSON.parse(item);
    Taro.showLoading({
      title:'请稍等...',
      mask:true
    })
    console.log(333,row.name)
    const res = await callCheckEpidemicSurveyApi({
      personId:row.personId,//就诊人id
    ...
    if(res.code == 200) {
      if (res.data) {
        ...
        Taro.navigateTo({
          url: `/pages/home/write-patient-info/writePatientInfo?item=${JSON.stringify({...$item})}&personId=${row.personId}&userType=2`
        })
      } else {
        Taro.navigateTo({
          url: `/pages/home/epidemic/index?item=${JSON.stringify({sourceId,payType:0,docUrl:'', orgId, date, appointId,comboName, orgName, timeType, price,...row})}&personId=${row.personId}&userType=2`
        })
      }
    }
    Taro.hideLoading();
  ...
  addPatient = () => {
    ...
    const {sourceId, orgId, date, appointId,comboName, orgName, timeType, price} = JSON.parse(item);
    Taro.navigateTo({
      url: `/pages/home/insert-patient-info/index?comboName=${comboName}`
    })
  }
